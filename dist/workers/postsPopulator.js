            ...
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        ...
                        console.log('PostsPopulator.process', { type: type_1, posts: posts });
                        if (!posts.length) {
                            return [2 /*return*/, false];
                        }
                        return [4 /*yield*/, Promise.all(posts.map(function (post) {
                                return _this.agent.get("https://dj96a3dxm6.execute-api.us-east-1.amazonaws.com/prod?url=" + post.url).then(function (response) {
                                    ...
                                    return _this.pgPool.query("INSERT INTO posts (title, url, type, image, text) VALUES ($1, $2, $3, $4, $5)", [post.title, post.url, type_1, top_image, encodeURIComponent(text)]);
                                });
                            }))];
                    case 1:
                        _b.sent();
